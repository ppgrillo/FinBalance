...
export const Dashboard: React.FC<Props> = ({ user, onMenuClick }) => {
  ...
  const handleSweepSave = async () => {
      ...
      try {
          ...
          alert("¡Ahorro realizado con éxito!");
          setIsSweepOpen(false);
          await loadData(); 
      } catch (e) {
          alert("Error al mover fondos.");
      }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className={`bg-primary text-white p-6 rounded-3xl shadow-lg relative overflow-hidden w-full transition-all duration-500 ease-in-out ${isBalanceExpanded ? 'row-span-2' : ''}`}>
          ...
          <div className="relative z-10">
            ...
            >
                {isBalanceExpanded ? 'Ocultar Desglose' : 'Ver Salud Presupuestal'}
                    ...
                    <div className="bg-black/10 p-4 rounded-2xl">
                         <div className="flex justify-between items-center mb-2">
                             <h3 className="font-bold text-sm flex items-center gap-2">
                                <Icons.Brain size={16} /> Planificación
                             </h3>
                             <Link to="/budgets" className="text-xs underline opacity-80">Ajustar</Link>
                         </div>
                         
                         <div className="flex justify-between text-xs mb-1 opacity-90">
                             <span>Presupuestos vs Techo Dinámico ($ {safeToSpendLimit.toLocaleString()})</span>
                             <span>${totalBudgetedSum.toLocaleString()}</span>
                         </div>
                         <div className="w-full bg-black/20 h-1.5 rounded-full overflow-hidden mb-2">
                             <div 
                                className={`h-full ${isOverPlanned ? 'bg-red-400' : 'bg-green-400'}`}
                                style={{ width: `${Math.min((totalBudgetedSum/Math.max(1, safeToSpendLimit))*100, 100)}%` }}
                             ></div>
                         </div>
                         
                         <p className="text-[10px] leading-tight opacity-80">
                             {isOverPlanned 
                                ? `⚠️ Tus presupuestos exceden lo que te queda libre después de restar tus metas de ahorro. Te recomendamos reducir gastos.` 
                                : `✅ Estás cubierto. Tienes $${unallocated.toLocaleString()} libres incluso después de tus metas.`
                             }
                         </p>
                    </div>

                    <div>
                        <h3 className="font-bold text-sm mb-3">Ejecución Real ({periodLabel})</h3>
                        <div className="grid gap-3">
                            {budgets.map((b, i) => (
                                <div key={i} className="bg-white/10 p-3 rounded-xl backdrop-blur-sm hover:bg-white/20 transition-colors">
                                    <div className="flex justify-between text-sm mb-2">
                                        <span className="font-medium">{b.category}</span>
                                        <span className="opacity-90">${b.spent.toLocaleString()} / ${b.limit.toLocaleString()}</span>
                                    </div>
                            ...
                            {budgets.length === 0 && <p className="text-xs opacity-70">No has definido presupuestos aún.</p>}
                        </div>
                    </div>
                </div>
            )}
