...
export const RecurringExpenses: React.FC = () => {
    ...
    const [sortBy, setSortBy] = useState<'date' | 'category' | 'amount' | 'name'>('date');
    ...
    const refreshData = async () => {
        ...
        try {
            ...
            if (catsData && catsData.length > 0) {
                ...
                setCategoriesList(Object.values(CategoryType).map(c => ({ name: c, color: '#9ca3af' })));
            }
        } catch (e) {
            console.error(e);
        } finally {
    ...
    const getCategoryColor = (catName: string) => {
        ...
        return cat?.color || '#6b7280'; // Default gray-500
    };
    ...
    const handleEdit = (item: RecurringItem) => {
        ...
        setNextPaymentDate(item.nextDate);
        setIsVariable(item.isVariable || false);
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingId) {
                await dbService.updateRecurringExpense(editingId, newItem);
            } else {
            ...
            error("Error al guardar. Asegúrate de la conexión a Supabase.");
        }
    };
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                {[
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${sortBy === opt.id ? 'bg-primary text-white shadow-md' : 'bg-white text-gray-500 hover:bg-gray-50'}`}
                    >
                        {opt.label}
                    </button>
                ))}
            ...
            <div className="grid gap-4">
                {sortBy === 'category' ? (
                    ...
                    Object.entries(items.reduce((acc, item) => {
                        ...
                        return acc;
                    }, {} as Record<string, RecurringItem[]>)).map(([catName, catItems]: [string, RecurringItem[]]) => (
                        <div key={catName} className="space-y-3">
                            <h3 className="text-lg font-bold text-textPrimary ml-1 sticky top-0 bg-background/95 backdrop-blur py-2 z-10">{catName}</h3>
                            {catItems.map(item => (
                                <div
                                    key={item.id}
                ...
                <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold">{editingId ? 'Editar Gasto Fijo' : 'Nuevo Gasto Fijo'}</h2>
                    ...
                    <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-fade-in">
                        ...
                        <div className="flex gap-3 mt-6">
                            <button
                                ...
                                className="flex-1 py-3 rounded-xl bg-primary text-white font-semibold hover:bg-purple-600 shadow-lg shadow-primary/20 flex items-center justify-center gap-2"
                            >
                                <Icons.Edit size={20} />
                                <span>Editar</span>
                            </button>
                            <button
                                ...
                                className="w-14 flex items-center justify-center rounded-xl bg-red-50 text-red-500 hover:bg-red-100 transition-colors"
                            >
                                <Icons.Trash size={20} />
                            </button>
                        </div>
