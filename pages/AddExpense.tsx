...
export const AddExpense: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (transactionType !== 'transfer' && isCreatingLoan && !newLoanName.trim()) {
        alert("Ingresa el nombre de quién te prestó el dinero.");
        return;
    }
    ...
    try {
        ...
        if (transactionType !== 'transfer' && isCustomCategory && category && !availableCategories.includes(category)) {
  ...
  const handleAnalyze = async (text?: string, base64Img?: string) => {
    setLoading(true);
    try {
      const result = await geminiService.analyzeExpense(text, base64Img);
      if (result.amount) setAmount(result.amount.toString());
      if (result.category) {
          const match = availableCategories.find(c => c.toLowerCase() === result.category?.toLowerCase());
          setCategory(match || result.category);
      }
      ...
      {transactionType !== 'transfer' && (
        <div className="bg-white p-1 rounded-xl flex mb-6 shadow-sm">
            ...
            <button
                key={m}
                onClick={() => setMode(m)}
                className={`flex-1 flex flex-col items-center py-3 rounded-lg text-xs font-medium transition-all ${
                mode === m ? 'bg-primary text-white shadow-md' : 'text-gray-400 hover:bg-gray-50'
                }`}
            >
            ...
            {transactionType !== 'transfer' && (
                <>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {!isCustomCategory ? (
                            <>
                                {/* Show different categories based on type if desired, keeping mixed for now */}
                                ...
                                <button
                                    ...
                                    category === cat 
                                        ...
                                        : 'bg-background border-transparent text-textSecondary hover:bg-gray-100'
                                    }`}
                                >
                                    {cat}
                                </button>
                                ...
                                {availableCategories.length > 5 && (
                                    <select 
                                        value={availableCategories.slice(0,5).includes(category) ? "" : category}
                                        onChange={(e) => {
                                            if (e.target.value) setCategory(e.target.value);
                                        }}
                                        className="py-2 px-3 rounded-lg text-xs font-medium bg-background border border-transparent text-textSecondary outline-none focus:border-primary"
                                    >
                                        <option value="" disabled>Más...</option>
                                        {availableCategories.slice(5).map(cat => (
                                ...
                                <button
                                    ...
                                    className="py-2 px-3 rounded-lg text-xs font-medium text-left transition-all border bg-background border-dashed border-gray-300 text-textSecondary hover:border-primary hover:text-primary"
                                >
                                    + Otra...
                                </button>
                            </>
                            ...
                            <div className="col-span-2 flex gap-2">
                                ...
                                <button 
                                    ...
                                    className="bg-gray-200 p-3 rounded-xl text-gray-600"
                                >
                                    <Icons.Close />
                                </button>
                            </div>
            ...
            <div className="relative">
              ...
              <button 
                ...
                className={`w-24 h-24 rounded-full flex items-center justify-center shadow-xl transition-all ${
                  isRecording ? 'bg-red-500 text-white' : 'bg-primary text-white hover:scale-105'
                }`}
              >
                <Icons.Mic size={40} />
              </button>
