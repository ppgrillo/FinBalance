...
export const Goals: React.FC = () => {
  ...
  const handleAskAI = async () => {
    if (!newGoal.name || !newGoal.targetAmount || !newGoal.deadline) {
        alert("Por favor llena el nombre, monto y fecha objetivo para generar un plan.");
        return;
    }
    setLoadingPlan(true);
  ...
  const handleSave = async () => {
      // Robust validation
      if (!newGoal.name) { alert("Por favor escribe un nombre para la meta."); return; }
      if (!newGoal.targetAmount || newGoal.targetAmount <= 0) { alert("Por favor ingresa un monto objetivo válido."); return; }
      if (!newGoal.deadline) { alert("Por favor selecciona una fecha límite."); return; }
      ...
      try {
          ...
          console.error(e);
          alert('Error guardando meta: ' + (e.message || "Verifica tu conexión a internet."));
      }
  };
  ...
  const handleDeposit = async () => {
      ...
      try {
          ...
          setDepositAmount('');
          setSourceAccount('');
          alert("¡Aporte realizado con éxito!");
      } catch (e) {
