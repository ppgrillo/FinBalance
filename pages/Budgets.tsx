...
export const Budgets: React.FC = () => {
  ...
  const handleDelete = async () => {
      ...
      if (window.confirm(`¿Eliminar presupuesto de ${editingBudget.category}?`)) {
          try {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (isCreatingCategory && newCategoryName.trim()) {
            // New category logic handled by dbService.addBudget using ensureCategoryExists internally now
        ...
        if (editingBudget) {
            ...
            if (budgets.some(b => b.category === finalCategory)) {
                alert("Esta categoría ya tiene un presupuesto.");
                return;
            }
