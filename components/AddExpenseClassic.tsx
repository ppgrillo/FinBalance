...
export const AddExpenseClassic: React.FC = () => {
    ...
    const startListening = () => {
        ...
        if (!SpeechRecognition) {
            warning("Tu navegador no soporta reconocimiento de voz. Intenta usar Chrome o Safari.");
            return;
        }
        ...
        recognition.onerror = (event: any) => {
            ...
            if (event.error === 'not-allowed') {
                ...
                error("Error al escuchar. Intenta de nuevo.");
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const isSuccess = await saveTransaction();
        if (isSuccess) {
            ...
            if (expenseQueue.length > 0 && currentQueueIndex < expenseQueue.length - 1) {
                ...
                success("Guardado. Siguiente...");
            } else {
                success("Transacción guardada exitosamente");
                navigate('/dashboard'); // Changed to dashboard since / is now AddExpense
            }
        }
    };
    ...
    const saveTransaction = async (): Promise<boolean> => {
        ...
        if (transactionType === 'transfer' && (!destAccount || destAccount === selectedAccount)) {
            warning('Selecciona una cuenta de destino diferente al origen');
            return false;
        }

        if (transactionType !== 'transfer' && isCreatingLoan && !newLoanName.trim()) {
            warning("Ingresa el nombre del préstamo o acreedor.");
            return false;
        }

        if (!selectedAccount && !isCreatingLoan && accounts.length > 0) {
            warning('Selecciona una cuenta');
            return false;
        }
        ...
        try {
            ...
            error('Error al guardar la transacción: ' + (e.message || "Error desconocido"));
            return false;
        } finally {
            setLoading(false);
        }
    ...
    const handleAnalyze = async (text?: string, base64Img?: string) => {
        ...
        try {
            ...
            if (results && results.length > 0) {
                ...
                success(`Se encontraron ${results.length} movimientos`);
            } else {
                warning("No se pudo entender la transacción.");
            }
    ...
    const takePhoto = async () => {
        try {
            ...
            if (image.base64String) {
                handleAnalyze(undefined, image.base64String);
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <h1 className="text-2xl font-bold">Registrar (Clásico)</h1>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm p-6 min-h-[400px] relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {/* Date Input */}
                    <div>
                        <label className="block text-xs font-bold text-textSecondary uppercase tracking-wider mb-2">Fecha</label>
                        <input
                            type="date"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    >
                                        ...
                                        <optgroup label="Deudas y Créditos">
                                            {accounts.filter(a => a.type === 'Credit' || a.type === 'Loan').map(acc => (
                                                <option key={acc.id} value={acc.id}>{acc.name} (${acc.balance})</option>
                                            ))}
                                        </optgroup>
                                ...
                                <div>
                                    ...
                                    >
                                        <option value="" disabled>Seleccionar...</option>
                                        <optgroup label="Mis Cuentas">
                                            {accounts.filter(a => a.id !== selectedAccount && (a.type !== 'Credit' && a.type !== 'Loan')).map(acc => (
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transform transition-all active:scale-95 ${loading ? 'bg-gray-400 cursor-not-allowed' :
                            transactionType === 'income' ? 'bg-green-500 hover:bg-green-600 shadow-green-500/30' :
                                transactionType === 'transfer' ? 'bg-blue-500 hover:bg-blue-600 shadow-blue-500/30' :
                                    'bg-primary hover:bg-purple-600 shadow-purple-500/30'
                            }`}
                    >
                    ...
                    {expenseQueue.length > 0 && (
                        <div className="flex justify-between text-xs text-gray-400 mt-2">
                            <span>Movimiento {currentQueueIndex + 1} de {expenseQueue.length}</span>
                            <button type="button" onClick={handleSkip} className="hover:text-primary">Saltar este</button>
                        </div>
                    )}
                </form>
            </div>
